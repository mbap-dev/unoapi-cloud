  ...
  async consume(phone: string, data: object) {
    ...
    if (a.webhooks) {
      ...
      if (config.provider == 'forwarder') {
        ...
        if (!isUpdateMessage(payload) && payload.entry?.[0]?.changes?.[0]?.value?.messages) {
          const store = await config.getStore(phone, config)
          const { mediaStore } = store
          payload.entry[0].changes[0].value.messages = await Promise.all(
            payload.entry[0].changes[0].value.messages.map(async (message) => {
              const msg = await mediaStore.saveMediaFromPayload(message)
                if (!/^\d+$/.test(msg.from)) {
                  msg.from = jidToPhoneNumber(msg.from, '')
                }
              return msg
            })
          )
        }
        applyInstanceMessageIds(phone, payload)
      }
